    ...
    shell:
        ...
        if [ -f "$DB_FILE" ]; then
            ...
            if [ "$AVAIL_GB" -gt $(( DB_GB + 16 )) ]; then
                DBMEM="--dbmem"
                echo "[eggnog_viral] --dbmem ON (db ${{DB_GB}}G, RAM disponivel ${{AVAIL_GB}}G)" | tee -a {log}
            else
                echo "[eggnog_viral] --dbmem OFF (db ${{DB_GB}}G, RAM disponivel ${{AVAIL_GB}}G, precisa > $(( DB_GB + 16 ))G)" | tee -a {log}
            fi
        fi
