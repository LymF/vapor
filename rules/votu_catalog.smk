...
# ══════════════════════════════════════════════════════════════════════
# Global viral genome annotation — pharokka / phold / genome maps
#
# Moved from rules/annotation.smk on 2026-08-18, second half of "(h)
# Princípio: computar no representante, herdar no membro"
# (docs/ROADMAP_SIMPLIFICACAO.md). pharokka/phold/genome-map annotation
# depends only on the SEQUENCE of a vOTU representative, never on which
# sample it came from, so it runs ONCE over the global catalog instead of
# once per sample on a filtered subset of the same global FASTA.
#
# This also FIXES a correctness bug, not just a cost one (see roadmap
# "O bug de namespace de ID"): the old per-sample `pharokka` built its
# `hq_set` from that sample's OWN CheckV summary (bare contig IDs, e.g.
# "MEGAHIT_k141_10006") and used it to filter
# `votu_catalog_reps.output.mq_fasta`, whose headers are NAMESPACED
# ("{source_id}|{contig_id}", e.g. "S1|MEGAHIT_k141_10006"). Bare vs.
# namespaced never matched, so `hq_set` always filtered the global FASTA
# down to zero sequences -- every sample silently produced
# "No HQ phages found" and cascaded empty inputs into phold and the
# genome maps. The fix is not to add the prefix into that join (a
# representative from S2 would then be accepted/rejected by S1's CheckV,
# which is the wrong semantics for a global vOTU); it is to gate on the
# catalog's OWN completeness (`_load_catalog_completeness()`, already
# namespaced) instead of any one sample's CheckV.
# ══════════════════════════════════════════════════════════════════════


rule votu_catalog_quality_summary:
    """CheckV-shaped quality table for the whole catalog, namespaced IDs.

    genome_map_universal.py is a standalone script invoked via `shell:`, so
    it cannot call `_load_catalog_completeness()` directly the way the
    `run:`-based rules below do -- it needs an actual TSV file, with the
    same (contig_id, completeness, checkv_quality) shape CheckV's own
    quality_summary.tsv has (the script only ever reads those two of the
    three columns). This is "option (a)" from the (h) spec: consolidate
    once into a catalog-shaped file rather than teach the script a second
    input format. Covers EVERY namespaced ID with a CheckV row (not just
    MQ+ reps), since votu_genome_map_virus ranks candidates itself.
    """
    input:
        checkv = _catalog_checkv_summaries,
    output:
        tsv = f"{CATALOG_DIR}/checkv/catalog_quality_summary.tsv",
    log:
        f"{OUTDIR}/logs/votu_catalog_quality_summary.log"
    run:
        completeness, quality = _load_catalog_completeness()
        os.makedirs(os.path.dirname(str(output.tsv)), exist_ok=True)
        with open(str(output.tsv), "w") as fh:
            fh.write("contig_id\tcompleteness\tcheckv_quality\n")
            for cid in sorted(completeness):
                fh.write(f"{cid}\t{completeness[cid]:.6g}\t{quality.get(cid, '')}\n")
        with open(str(log[0]), "w") as lf:
            lf.write(f"[votu_catalog_quality_summary] namespaced IDs written: {len(completeness)}\n")


rule votu_catalog_genomad_genes:
    """Aggregate every source's GeNomad *_genes.tsv into one namespaced table.

    Same source enumeration and prefixing convention as
    `_load_catalog_genomad()` (which aggregates *_virus_summary.tsv for
    votu_taxonomy), applied to the *_genes.tsv gene-position table instead.
    GeNomad gene IDs are UNDERSCORE-delimited, "{contig_id}_{N}" (bare
    contig_id) -- prodigal-gv numbering, NOT the "{contig_id}|{N}" this
    docstring used to claim. That false claim is what put a `rsplit("|", 1)`
    into genome_map_universal.py::_parse_genomad_for_genome and silently
    emptied every virus map once the catalog started namespacing IDs; that
    function now matches on the `"{genome_id}_"` prefix instead.
    Re-namespacing here therefore means prefixing the WHOLE gene id with
    "{source_id}|": "{contig}_{N}" becomes "{source_id}|{contig}_{N}",
    which starts with "{source_id}|{contig}_" -- exactly the pool's
    namespaced contig ID followed by the gene separator, so the match in
    _parse_genomad_for_genome lands.
    All other columns (start/end/strand/annotation_*) are per-gene
    attributes and pass through unchanged.
    """
    input:
        genomad_done = _catalog_genomad_done_files,
    output:
        tsv = f"{CATALOG_DIR}/genomad/catalog_genes.tsv",
    log:
        f"{OUTDIR}/logs/votu_catalog_genomad_genes.log"
    run:
        import csv, glob
        ...
        fieldnames = None
        rows_out = []
        n_sources_with_genes = 0
        for source_id, done_path in _catalog_genomad_pairs():
            gdir = os.path.dirname(done_path)
            # *_virus_genes.tsv, nao *_genes.tsv: o glob solto casa tambem
            # *_plasmid_genes.tsv, *_provirus_genes.tsv e o _genes.tsv do
            # annotate, e gfiles[0] escolhia por ordem de diretorio.
            gfiles = glob.glob(os.path.join(gdir, "**", "*_virus_genes.tsv"), recursive=True)
            if not gfiles:
                continue
            n_sources_with_genes += 1
            with open(gfiles[0]) as fh:
                rdr = csv.DictReader(fh, delimiter="\t")
                ...
                for row in rdr:
                    ...
                    if not gene_id:
                        continue
                    row = dict(row)
                    row["gene"] = f"{source_id}|{gene_id}"
                    rows_out.append(row)

        with open(str(log[0]), "w") as lf:
            lf.write(f"[votu_catalog_genomad_genes] sources with *_genes.tsv: "
                     f"{n_sources_with_genes}\n")
            lf.write(f"[votu_catalog_genomad_genes] gene rows written: {len(rows_out)}\n")

        if fieldnames is None:
            # No source produced a genes TSV at all -- empty output, still
            # a valid (header-only) input for votu_genome_map_virus.
            open(str(output.tsv), "w").close()
            return
...
rule votu_pharokka:
    ...
    Selection criteria (unchanged from the per-sample rule):
      ...
      - No cap on genome count: every qualifying vOTU representative is
        annotated, since the resulting PHROGS hallmark-gene hits are what
        genome_map_universal.py uses to decide phage vs. non-phage virus.
    ...
    Output GBK/TSV are later used by votu_phold and genome_map_universal.py.
...
rule votu_genome_map_phage:
    """
    Circular genome maps for confirmed phages (PHROGS color scheme), once
    for the whole vOTU catalog. Moved from the per-sample `genome_map_phage`
    (rules/annotation.smk) on 2026-08-18.

    `--checkv` now points at votu_catalog_quality_summary's namespaced,
    catalog-wide table instead of one sample's CheckV summary -- the same
    fix as votu_pharokka, needed here because the input FASTA
    (votu_catalog_reps.output.mq_fasta) has always been namespaced while
    the per-sample CheckV summary never was.
    """
    input:
        phold_done   = rules.votu_phold.output.done,
        phold_gbk    = rules.votu_phold.output.gbk,
        pharokka_tsv = rules.votu_pharokka.output.tsv,
        checkv       = rules.votu_catalog_quality_summary.output.tsv,
        viral_nr     = rules.votu_catalog_reps.output.mq_fasta,
    output:
        done = f"{CATALOG_DIR}/annotation/genome_maps/phage_maps_done.txt",
    log:
        f"{OUTDIR}/logs/votu_genome_map_phage.log"
    benchmark:
        f"{OUTDIR}/benchmarks/votu_genome_map_phage.tsv"
    conda: "../envs/env_annotation.yaml"
    container:  CONTAINERS.get("genome_map")
    threads: 1
    params:
        outdir      = f"{CATALOG_DIR}/annotation/genome_maps/phage",
...
rule votu_genome_map_virus:
    """
    Circular genome maps for non-phage viral sequences, once for the whole
    vOTU catalog. Moved from the per-sample `genome_map_virus`
    (rules/annotation.smk) on 2026-08-18 -- this was the second BUG
    confirmed in the roadmap analysis, independent of pharokka: `comp_map`
    came from a sample's bare-ID CheckV summary while `seqs` came from the
    namespaced global FASTA, so `comp_map.get(gid, 0.0)` was always 0.0 and
    zero genomes ever passed `min_comp`.

    Three inputs fixed for the namespace, each documented separately:
      --checkv         votu_catalog_quality_summary.output.tsv (namespaced
                        contig_id, same fix as votu_genome_map_phage).
      --genomad-genes  votu_catalog_genomad_genes.output.tsv: aggregates
                        every source's *_genes.tsv and re-prefixes the
                        pipe-delimited gene id with "{source_id}|" so its
                        rsplit("|", 1) contig recovers the pool's namespaced
                        contig ID (see that rule's docstring).
      --viral-taxonomy rules.votu_taxonomy.output.tsv: this one was ALREADY
                        namespaced (seq_name = "{source_id}|{contig}",
                        votu_catalog.smk votu_taxonomy docstring) -- no
                        per-sample equivalent existed for this rule to have
                        used before, since taxonomy itself only became a
                        global table earlier in (h).
    """
    input:
        checkv        = rules.votu_catalog_quality_summary.output.tsv,
        viral_nr      = rules.votu_catalog_reps.output.mq_fasta,
        phage_done    = rules.votu_genome_map_phage.output.done,
        genomad_genes = rules.votu_catalog_genomad_genes.output.tsv,
        viral_tax     = rules.votu_taxonomy.output.tsv,
    output:
        done = f"{CATALOG_DIR}/annotation/genome_maps/virus_maps_done.txt",
    log:
        f"{OUTDIR}/logs/votu_genome_map_virus.log"
    benchmark:
        f"{OUTDIR}/benchmarks/votu_genome_map_virus.tsv"
    conda: "../envs/env_annotation.yaml"
    container:  CONTAINERS.get("genome_map")
    threads: 1
    params:
        outdir      = f"{CATALOG_DIR}/annotation/genome_maps/virus",
        phage_dir   = f"{CATALOG_DIR}/annotation/genome_maps/phage",
        min_comp    = GENOME_MAP_MIN_COMP_VIRAL,
        top_n       = GENOME_MAP_TOP_N,
        scripts_dir = SCRIPTS_DIR,
    run:
        ...
        # Wipe stale output -- same rationale as the removed per-sample rule:
        # a relative script path used to fail silently if cwd wasn't the
        # pipeline directory, leaving old SVGs around from a prior
        # run/classification logic. Always start clean.
        ...
        with open(log_path, "w") as lf:
            lf.write(f"[votu_genome_map_virus] confirmed phages to exclude: "
                     f"{len(confirmed_ids)}\n")

        genomad_genes_arg = str(input.genomad_genes)
        has_genomad = os.path.exists(genomad_genes_arg) and os.path.getsize(genomad_genes_arg) > 0
        ...
        shell(
            ...
            " --mode virus"
            " --fasta {input.viral_nr}"
            + (" --genomad-genes " + genomad_genes_arg if has_genomad else "") +
            " --checkv {input.checkv}"
