...
- **rules/**: Directory containing modular Snakemake rules, each handling a specific functional block:
  ...
  - `annotation.smk`: Prokka + eggNOG-mapper + KO extraction on prokaryotic MAGs — **global over the catalog representatives since 2026-08-19**. **Bakta was replaced by Prokka on 2026-09-09** (`mag_prokka`, container-only DB-free, own `env_prokka.yaml`): Bakta 1.x hung for days inside `diamond blastp` against `psc.dmnd`. Do not reintroduce `mag_bakta`, `bakta_db` or the `annotation/bakta/` paths. `mag_eggnog_prok` prefixes every protein with `{genome}__` when concatenating the Prokka FAAs (Prokka's own IDs are locus tags like `LLOGBO_00001`, which carry no link to the MAG) and writes `eggnog_annotations.tsv` with emapper's `##` preamble stripped — a plain DictReader took `## <date>` as the header and returned None for every column, which is why the report's COG chart counted every protein as "Function unknown". `ko_per_mag.tsv`'s `mag` column is the real genome now, resolved against known genomes, not the regex-derived locus-tag prefix it used to be. Since 2026-08-19 the same rule also writes `cazy_per_mag.tsv` from the emapper **column 19 (`CAZy`)** — that column had always been in the file and no consumer read it; extracting it here costs **+0 jobs** and no new database (a protein carrying only CAZy and no KO is now kept, which the old `continue`-on-missing-KO discarded). `mag_kegg_completeness` then turns the KOs into KEGG module completeness with **kegg-pathways-completeness** (bioconda 1.4.4, in `env_annotation.yaml` — no new env): 573 modules bundled in the package, no database download. It was chosen over KEGG-Decoder because that one is **not in bioconda** and because it takes the genome as a **column**, not a prefix cut at the first underscore — KEGG-Decoder would have turned `S1__binette_bin1` into `S1` and credited every module to the SAMPLE. Verified against the real v1.4.4: names round-trip intact. Its per-MAG output keeps `missing_ko`, so an incomplete pathway is interpretable. pharokka and phold moved to `votu_catalog.smk` on 2026-08-18 (principle "(h)"). **The genome maps were removed entirely on 2026-08-19** (`genome_map_prok`, `votu_genome_map_phage`, `votu_genome_map_virus`, `scripts/genome_map*.py`, the report panel, the `genome_map_*` config keys) — they took with them `votu_catalog_quality_summary` and `votu_catalog_genomad_genes`, which existed only to feed them.
  ...
  - `pangenome.smk`: Fase 1 do pangenoma dos clusters do catálogo global de MAGs, since 2026-08-19 — see `docs/superpowers/specs/2026-08-19-pangenoma-clusters-defesa-design.md`. Selects a small subset of clusters for per-MEMBER annotation (everything else in the catalog runs on the representative only): a cluster qualifies only with **>= 3 members** AND (a defense island OR **>= 3** defense systems OR a consensus ARG, `n_tools >= 2`) — PlasmidFinder is recorded as a mobility signal in `candidates.tsv` but never qualifies a cluster on its own. Selection follows the `mag_prokka` pattern (a data-dependent subset picked inside a job of fixed count, via `qualifying_bins.txt`-equivalent files) and deliberately **never a `checkpoint`** — a dynamic DAG would break the dry-run invariant `docs/ROADMAP_SIMPLIFICACAO.md`'s verification method depends on. `mag_pangenome_defensefinder`/`amrfinderplus`/`rgi_card`/`deeparg`/`argnorm`/`amr_consensus` are inherited from the `mag_*` global rules via `use rule ... as ... with:` (input/output swapped to the member-level manifest, body untouched) and chain only with each other's `mag_pangenome_*` outputs, never with the global `mag_*` ones — reattaching to a global output would mix member-level and representative-level results silently. `mag_pangenome_matrix` writes `gene_by_member.tsv` in **three states**, not two: `x` (present), `.` (absent), `?` (member below the 70% completeness floor — not evaluable, and excluded from the frequency denominator, since an unassembled region in an incomplete MAG is not evidence the organism lacks the gene).
...
- CheckV, VirSorter2, geNomad, INPHARED, CheckM2, GTDB-Tk, pharokka, phold, eggNOG, CARD, DeepARG, DefenseFinder models, dbAPIS, sylph-tax.
