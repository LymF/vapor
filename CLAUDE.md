...
- **rules/**: Directory containing modular Snakemake rules, each handling a specific functional block:
  ...
  - `host_prediction.smk`: Phage-host prediction using PHIST — over the **global vOTU catalog** (`votu_catalog_reps.mq_fasta`) against the sample's own MAGs. **The group twin does NOT**: `coassembly_phist` (`coassembly.smk`) runs the GROUP's own `votu_mq_reps.fasta` against that group's VAMB MAGs, so its phage IDs are bare group contigs, not `{source}|{contig}` — never join its output against catalog IDs. Docs claimed otherwise until 2026-08-19. PHIST is really `kmer-db build` + `kmer-db new2all` + `phist`, and kmer-db emits one result row per input **file**, which is why `split_viral_fastas.py` exists: both sides need one FASTA per genome plus a list file. Note these results describe assembled contigs (`k141_…`); they share no key with the sylph track's IMG/VR reference genomes (`t__IMGVR_UViG_…`).
